...
QUEUE_p_t create_queue(int *items, int numItems) {
 ...
 CALLOC_ERROR:
  fprintf(stderr, "!!! calloc error in create_queue\n");
  return NULL;
...
PQUEUE_p_t create_p_queue(PQ_ITEM_t items[], int numItems) {
  ...
  if(NULL == q) {
    fprintf(stderr, "!!! calloc error in create_p_queue\n");
    return NULL;
  }
...
void enqueue(QUEUE_p_t queue, int item) {
  ...
  if(NULL == n) {
    fprintf(stderr, "!!! calloc error in enqueue\n");
    return NULL;
  }
...
void p_enqueue(PQUEUE_p_t queue, PQ_ITEM_t item) {
  ...
  if(NULL == n) {
    fprintf(stderr, "!!! calloc error in p_enqueue\n");
    return NULL;
  }
