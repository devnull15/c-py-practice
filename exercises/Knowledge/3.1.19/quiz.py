...
class question:
    ...
    def ask_if_redo(self):
        choices = ("Skip","Redo")
        if isinstance(self.q_options, list):
            possible_choices = "\n".join(map(str, self.q_options))
        else:
            possible_choices = str(self.q_options)
        user_question = "This item already exists. \n\nPrevious response was: {}\n\nPossible Choices:\n{}\n\nRedo?\n{}\n".format(self.q_resp,possible_choices,self.template)
        user_response = self.wrap_choice(user_question,choices)[1]
        self.stdscr.clear()

        return user_response == choices[0]
    ...
    def handle_true_false(self):
        ...
        if self.q_soln != None:
            self.q_correct = str(self.q_resp).lower() == str(self.q_soln).lower()
    ...
    def ask_question(self):
        if self.mode == MODE_CURSES:
            ...
            self.stdscr.clear()
            if self.q_resp != "" and self.q_resp != None:
                if self.ask_if_redo():
                    return
...
class quiz:
    ...
    def add_question(self,q_type,text,options=False,solution=False,resp=None):
...
class quiz_builder:
    ...
    def save_results(self,detail=DETAIL_WITH_RESPONSES):
        #print("Saving Results to: {}".format(self.out_file))
